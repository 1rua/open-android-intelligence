...
协议默认只运行在 HTTPS 上，使用普通请求/响应、SSE 和事件游标；用户显式输入的明文 HTTP 是持续警告的降级路径，不携带可核验的 Gateway 身份，也不得被表述为已固定身份（ADR 0047）；V1 不提供等价 WebSocket 通道。
...
连接开始时双方固定协议主次版本、认证方式、消息和附件版本、事件游标版本、能力 Schema、附件暂存期限、队列限制和功能标志。附件字节上限与 MIME allowlist 不属于 Gateway 协商策略；主版本或未知高风险语义不兼容时拒绝，次版本只使用显式交集。
...
Gateway 不设置附件字节数上限或 MIME allowlist；它把原始媒体和描述元数据交给 Agent 宿主，由 Agent 决定格式和处理大小。Android 与 Gateway 采用账号隔离的加密暂存和有界缓冲流式传输，并核对实际字节数与 SHA-256，避免整文件驻留内存。磁盘、网络和代理等底层资源失败必须明确上报，不伪装成业务大小限制。

附件先进入 Android 和 Gateway 的加密暂存区，长度与摘要一致后才可交付宿主。宿主确认、失败、解除配对或期限到期都会删除暂存内容；媒体类型仅作为元数据传递。
