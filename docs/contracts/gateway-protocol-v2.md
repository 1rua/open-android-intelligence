...
摘要不匹配是**破坏性升级**，不是可重试的瞬时故障：被拒的客户端在重新构建到同一份 Schema 之前完全无法连接，而这次拒绝发生在认证之前，因此不会留下会话、审计或事件记录。两端都必须让拒绝本身可诊断：

- 客户端必须给出「两端版本不一致」的专门说明，并指向升级动作；不得退化成通用连接失败文案——那会把用户引向排查网络。只能拿到 HTTP 状态码的路径（例如登录时协商已失效）同样必须命中这条说明，因为 406 是该错误的专属状态码。
- Gateway 在拒绝协商时必须记录一条警告，至少覆盖摘要不一致：包含发起方 `installationId`、客户端 `appVersion` 与两侧摘要；宿主若还知道具体成因（缺少必备能力、同一 `negotiationId` 被换成不同请求体等），应与该成因记在同一条警告里。

摘要本身是公开的契约哈希，不是秘密，但日志只记录 `sha256:` 加前八位十六进制；取值缺失时必须如实标成缺失。该字段已被请求 Schema 钉死为 `sha256:` 加 64 位小写十六进制，因此截断不会把任意客户端文本写进日志。
