...
const loadSharedFixtureRegistry = (
  ...
  if (
    ...
    candidate.catalogEntries.length !== 16 ||
  ...
  if (bindings.length !== 16) throw new Error("INVALID_FIXTURE_REGISTRY");
...
describe("Gateway Protocol v2 golden vector contract", () => {
  ...
  it("requires the one shared dispatched fixture registry before constructing its validator", () => {
    ...
    expect(sharedFixture.entries).toHaveLength(16);
    expect(sharedFixture.bindings).toHaveLength(16);
